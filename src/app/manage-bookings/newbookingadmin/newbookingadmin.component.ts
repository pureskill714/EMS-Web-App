...
export class NewbookingadminComponent implements OnInit {
  ...
  search(): void {
    ...
    this.http.post<any>('http://34.30.66.94/api/retrievebookingtimeslots', bodyData)
      .subscribe(
        (resultData: any) => {
          console.log(resultData);
      ...
      this.http.post<any>('http://34.30.66.94/api/retrievecalendardetails', bodyData)
      .subscribe(
        (resultData: any) => {
          console.log(resultData);
    ...
    if (this.selectedTimeSlots.length === 0) {
      ...
      dialogRef.afterClosed().subscribe(result => {
        if (result === 'confirm') {
          ...
          this.http.post("http://34.30.66.94/api/booking", bodyData).subscribe(
          (resultData: any) => {
              console.log(resultData);
